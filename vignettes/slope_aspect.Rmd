...
Current problem: The r.apply call including the redimensioning can run a long time. It might also occur that the query will not finish. In this case you can look up the temporary arrays that might have been created during the process. In one case the temporary array was created like the following: "__temp_srtm_africa_prep_120395534".

As a secondary approach we will perform the internal processes for the renaming and redimensioning manually, and run the AFL commands on via iquery directly on the server, to prevent connection losses.
```{r}
...
Currently, there is a problem with longterm queries and unintended connection losses that cannot be fixed in this package. If you encounter those problems, we recommend that you try to extract the appropriate AFL queries and run them with 'iquery' on the server (if you have access). Otherwise make sure to not loose the connection to the SHIM client, e.g.:
...
```{r}
# dev
script <- r.apply(x=srtm.ethiopia.prep,
                   f = slopeAspect,
                   array = "srtm_eth_slo_asp_test",
                   packages = c("raster","sp","rgdal"),
                   aggregates=c(),
                   output = list(dimy="double",dimx="double",band1="double",slope="double",aspect="double"),
                   eval=FALSE,
...
afl <- r.apply(x=srtm.ethiopia.prep,
                   f = slopeAspect,
                   array = "srtm_eth_slo_asp_test",
                   packages = c("raster","sp","rgdal"),
                   aggregates=c(),
