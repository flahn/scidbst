...
- the overlap in time is (number of years/4)+(1 or 2)
...
Now I state a first version of the function that will be applied to the data chunk using the previous lines of code. The procedures are the following:
...
-- use focal to create RasterLayers with the correct neighboring cells
...
linearRegressionNeighborhood = function(x, ...) {
    ...
    rebuildDataFrame = function(x) {
        b = cbind(y=coordinates(x$data)[,"dimy"],x=coordinates(x$data)[,"dimx"],t=x$t,getValues(x$data))
        as.data.frame(b)
    }
